...
namespace SistemaInventario.Areas.Admin.Controllers
{
    ...
    public class ProductoController : Controller
    {
        ...
        private readonly IWebHostEnvironment _webHostEnvironment; //Para acceder al directorio Root y por consiguiente a las Imagenes

        public ProductoController(IUnidadTrabajo unidadTrabajo, IWebHostEnvironment webHostEnvironment)
        {
            _unidadTrabajo = unidadTrabajo;
            _webHostEnvironment = webHostEnvironment; //Para acceder al directorio Root y por consiguiente a las Imagenes
        }
        ...
        public async Task<IActionResult> Upsert (ProductoVM productoVM)
        {
            ...
            {
                var files = HttpContext.Request.Form.Files; //captura los archivos que le pasemos mediante el post, en este caso las imagenes
                string webRootPath = _webHostEnvironment.WebRootPath; //aqui estamos capturando el directorio wwwroot

                if (productoVM.Producto.Id == 0)
                {
                    //Crear
                    string upload = webRootPath + DS.ImagenRuta;
                    string fileName = Guid.NewGuid().ToString(); //independientemente del nombre de la imagen, siempre se guardara como Id Unico
                    string extension = Path.GetExtension(files[0].FileName); //toma incluyendo el punto y lo que hay despues con GetExtension .jpg por ejemplo
                ...
                else
                {
                    ...
                    } //En el caso que no se mande una nueva imagen, se conservara la imagen anterior
                    else 
                    {
        ...
        {
            var todos = await _unidadTrabajo.Producto.ObtenerTodos(incluirPropiedades: "Categoria,Marca");//se incluyen las navegaciones para saber a que marca y a que categoria pertenecen y poder navegar en las tablas categoria y marca
